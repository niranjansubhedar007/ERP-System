...
export default function Sales() {
  ...
  const fetchSales = useCallback(() => {
    setLoading(true);
    api.get('/sales').then(r => setSaleOrders(r.data)).finally(() => setLoading(false));
  }, []);
  ...
  const handleDelete = async () => {
    try { await api.delete(`/sales/${deleteTarget.id}`); toast.success('Sale order deleted, inventory restored'); setDeleteTarget(null); fetchSales(); }
    ...
    <div>
      <PageHeader title="Sale / Outward" subtitle="Dispatch finished goods to customers"
        action={hasRole('admin','sales','warehouse') && (
          <button onClick={() => setShowCreate(true)} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700"><Plus size={16} /> New Sale Order</button>
        )}
      />
      {loading ? <Spinner /> : <Table columns={columns} data={saleOrders} emptyMessage="No sale orders yet" />}
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Create Sale / Outward Order" size="lg">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">Create Sale Order</button>
          </div>
        </form>
      </Modal>
